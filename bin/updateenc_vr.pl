...
#
# 2004/4/6	liangc	increase the enclosure to 20480 bytes
# 2004/4/6	liangc	change to upload enclosure for 'vr'
#			- the enclosure name is "vr_file_change.lst"
#
#################################################
################################################
#main
...
mkenc($objID,"vr_file_change.lst",$newstr);
...
sub fd_chg_lst
{
  ...
  my($cmdbuf,$sqlcmdbuf);
  $sqlcmdbuf = "format  %.20480s
   	     	 select text 
	      	from enclosures 
	      	where identifier='$prname' and name='vr_file_change.lst'";
...
{my $objPRID = shift @_;   #object PR's ID 
 ...
 my ($rtbuf,$cmdbuf);
 my $today = `$ENV{"DDTSHOME"}/bin/today`;
 chomp ($today) ;
